...
import com.purbon.kafka.julie.model.users.platform.*;
...
import static com.purbon.kafka.julie.Utils.asNullableStream;
...
public class AclsBindingsBuilder implements BindingsBuilderProvider {
    ...
    public AclsBindingsBuilder(AppConfig config) {
        this.config = config;
    }

    @Override
    public List<JulieAclBinding> buildBindingsForConnect(
            Connector connector, String topicPrefixNotInUse) {

        String principal = connector.getPrincipal();
        Stream<String> readTopics = asNullableStream(connector.getTopics().get("read"));
        Stream<String> writeTopics = asNullableStream(connector.getTopics().get("write"));
    ...
    @Override
    public List<JulieAclBinding> buildBindingsForStreamsApp(
            String principal,
            String topicPrefix,
    ...
    @Override
    public List<JulieAclBinding> buildBindingsForConsumers(
    ...
    @Override
    public List<JulieAclBinding> buildBindingsForProducers(
    ...
    @Override
    public List<JulieAclBinding> buildBindingsForSchemaRegistry(
            SchemaRegistryInstance schemaRegistry) {
        return toList(schemaRegistryAclsStream(schemaRegistry));
    }

    @Override
    public List<JulieAclBinding> buildBindingsForControlCenter(String principal, String appId) {
        return toList(controlCenterStream(principal, appId));
    }

    @Override
    public Collection<JulieAclBinding> buildBindingsForKSqlServer(KsqlServerInstance ksqlServer) {
        return toList(ksqlServerStream(ksqlServer));
    }

    @Override
    public Collection<JulieAclBinding> buildBindingsForKSqlApp(KSqlApp app, String prefix) {
        return toList(ksqlAppStream(app, prefix));
    }

    @Override
    public Collection<JulieAclBinding> buildBindingsForJulieRole(
            Other other, String name, List<JulieRoleAcl> acls) throws IOException {

        List<JulieAclBinding> bindings = new ArrayList<>();
        for (JulieRoleAcl acl : acls) {
            ...
            var binding =
                    new AclBuilder(other.getPrincipal())
                            ...
                            .build();
            bindings.add(new JulieAclBinding(binding));
        }
        return bindings;
    }

    private List<JulieAclBinding> toList(Stream<AclBinding> bindingStream) {
        return bindingStream.map(JulieAclBinding::new).collect(Collectors.toList());
    }
    ...
    private Stream<AclBinding> producerAclsStream(Producer producer, String topic, boolean prefixed) {
        ...
        if (!producer.getTransactionId().isBlank()) {
            var transactionId = producer.getTransactionId();
            bindings.add(
        ...
        if (!producer.getTransactionId().isBlank() || producer.getIdempotence()) {
            ResourcePattern resourcePattern =
    ...
    private Stream<AclBinding> consumerAclsStream(Consumer consumer, String topic, boolean prefixed) {
        ...
        return Stream.of(
                ...
                buildGroupLevelAcl(
                        principal,
                        evaluateResourcePattern(consumer.getGroup()),
                        evaluateResourcePatternType(consumer.getGroup()),
                        AclOperation.READ));
    }
